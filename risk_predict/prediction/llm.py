...
clinet = AsyncOpenAI(api_key=settings.OPENAI_API_KEY)

class RiskPredictionResult(BaseModel):
    diabetes_propability: float  # 당뇨 위험도
    hypertension_propability: float  # 고혈압 위험도
...
async def predict_health_risk(
    ...
    response = await clinet.responses.parse(
        model = model_version,
        input = prompt,
